...
const parseGraphqlSqlServer = require('./create_file_func/graphql_sql_server');
const parseSqlSchema = require('./create_file_func/sql_schema');
  ...
  buildDirectories(dateStamp, () => {
    ...
    buildClientQueries(data.data, dateStamp, () => {
      if (data.database === 'MongoDB') {
        ...
        buildForSql(data.data, dateStamp, () => {
          // ZIP main build folder and responds to client
          zipper.sync.zip(path.join(PATH, `build-files${dateStamp}`)).compress().save(path.join(PATH, `graphql${dateStamp}.zip`));
...
function buildForSql(data, dateStamp, cb) {
  fs.writeFileSync(path.join(PATH, `build-files${dateStamp}/server/graphql-schema/index.js`), parseGraphqlSqlServer(data));
  const indexes = Object.keys(data);
  
  indexes.forEach((index) => {
    parseSqlSchema(data[index], (query) => {
      fs.writeFileSync(path.join(PATH, `build-files${dateStamp}/server/db-model/${data[index].type.toLowerCase()}.js`), query);
    });
  });
  return cb();
